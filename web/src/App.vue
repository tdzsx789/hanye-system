...
const FREIGHT_QUOTE_TAB = "报价";
const SUPPLIER_COST_SHARED_DIRECTION = SHARED_DIRECTION;
...
const FEE_DRIVER_ROLE_OPTIONS = ["", "香港司机", "大陆骑师", "跟随订单司机", "手动指定"];
const FEE_DRIVER_ROLE_LABELS = {
  "": "未指定",
  香港司机: "香港司机",
  大陆骑师: "大陆骑师",
  跟随订单司机: "跟随订单司机",
...
const ORDER_STATUS_OPTIONS = ["待确认", "预排", "正常", "通关中", "待审核", "缺票据", "费用待确认", "已审核"];
const DEFAULT_DRIVER_TYPES = ["香港司机", "大陆骑师", "其他"];
const TEMPLATE_SYSTEM_TOTAL_COLUMNS = [
  { key: "__hkdTotal", label: "HKD合计", width: 64 },
  { key: "__rmbTotal", label: "RMB合计", width: 64 }
];
const TEMPLATE_SYSTEM_SEQUENCE_COLUMN = { key: "__sequence", label: "序号", width: 42 };
const TEMPLATE_SYSTEM_MANAGED_COLUMNS = [
  TEMPLATE_SYSTEM_SEQUENCE_COLUMN,
  ...TEMPLATE_SYSTEM_TOTAL_COLUMNS
];
const TEMPLATE_REMOVED_COLUMN_KEYS = new Set(["receivableHKD", "receivableRMB"]);
const TEMPLATE_ORDER_BASE_COLUMNS = [
...
const DRIVER_ADJUSTMENT_TYPES = ["预支款", "停车费", "过磅费", "加油费", "维修费", "其他报销"];
const DRIVER_ADJUSTMENT_STATUS_OPTIONS = ["待工资结算", "工资报销", "已结算"];
const TEMPLATE_PREVIEW_SAMPLE_ORDERS = [
  {
    no: "HY2606300001",
...
const RELATED_ORDER_DATE_FILTERS = [
  { key: "all", label: "全部" },
  { key: "today", label: "今日" },
  { key: "week", label: "本周" },
  { key: "month", label: "本月" },
...
const DISPATCH_LOAD_TIME_OPTIONS = Array.from({ length: 96 }, (_, index) => {
  const totalMinutes = index * 15;
  const hour = Math.floor(totalMinutes / 60);
  const minute = totalMinutes % 60;
  return `${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`;
});
const FREIGHT_DIRECTORY_LEVELS = [
...
const NEW_CONTACT_ROW_ID = "__new_contact__";
const CUSTOMER_ORDER_COLUMN_STORAGE_KEY = "hanye_customer_order_column_widths";
const ORDER_COLUMN_STORAGE_KEY = "hanye_order_column_widths";
const RELATED_VEHICLE_ORDER_COLUMN_STORAGE_KEY = "hanye_related_vehicle_order_column_widths";
const RELATED_DRIVER_ORDER_COLUMN_STORAGE_KEY = "hanye_related_driver_order_column_widths";
const CUSTOMER_ORDER_COLUMN_VISIBILITY_KEY = "hanye_customer_order_column_visibility";
const ORDER_COLUMN_VISIBILITY_KEY = "hanye_order_column_visibility";
const CUSTOMER_ORDER_COLUMN_ORDER_KEY = "hanye_customer_order_column_order";
const ORDER_COLUMN_ORDER_KEY = "hanye_order_column_order";
const CUSTOMER_ORDER_COLUMN_LOCKED_KEY = "hanye_customer_order_column_locked";
const ORDER_COLUMN_LOCKED_KEY = "hanye_order_column_locked";
const customerOrderColumns = reactive([
...
const orderColumns = reactive([
  { key: "select", label: "选择", width: 32, min: 30, locked: true },
  { key: "date", label: "日期", width: 88, min: 70 },
  ...
  { key: "plate", label: "车牌", width: 64, min: 48 },
  { key: "driver", label: "司机", width: 58, min: 44 },
  { key: "transportMode", label: "运输模式", width: 74, min: 54 },
  { key: "supplier", label: "外派供应商", width: 92, min: 64 },
  ...
  { key: "status", label: "状态", width: 72, min: 56 },
  { key: "actions", label: "操作", width: 150, min: 86, locked: true }
]);
const relatedVehicleOrderColumns = [
...
const relatedDriverOrderColumns = [
  { key: "no", label: "订单号", width: 104, min: 72 },
  { key: "date", label: "日期", width: 82, min: 68 },
  { key: "customer", label: "客户", width: 176, min: 90 },
  { key: "route", label: "路线", width: 260, min: 130 },
  { key: "transportMode", label: "运输模式", width: 82, min: 68 },
  { key: "baseFee", label: "基础趟费", width: 86, min: 70 },
  { key: "loadFee", label: "装货费", width: 76, min: 64 },
  { key: "unloadFee", label: "卸货费", width: 76, min: 64 },
  { key: "crossSeaFee", label: "过海费", width: 76, min: 64 },
  { key: "addPointFee", label: "加点费", width: 76, min: 64 },
  { key: "waitingFee", label: "等候费", width: 76, min: 64 },
  { key: "customerAdjust", label: "客户调整", width: 86, min: 70 },
  { key: "payable", label: "应付趟费", width: 88, min: 72 },
  { key: "status", label: "状态", width: 70, min: 60 }
];
customerOrderColumns.forEach((column, index) => { column.defaultIndex = index; });
orderColumns.forEach((column, index) => { column.defaultIndex = index; });
const financeWageTableColumns = [
  ...
  { key: "tripFee", label: "趟费合计", width: 150, min: 110 },
...
const customerListDetailColumns = reactive([
...
const vehicleListDetailColumns = reactive([
...
const driverListDetailColumns = reactive([
...
[
  ...
  customerListDetailColumns,
  vehicleListDetailColumns,
  driverListDetailColumns
].forEach((columns) => {
  columns.forEach((column, index) => { column.defaultIndex = index; });
});
const dataTableDensityOptions = [
...
function applySavedColumnOrder(columns, storageKey) {
  ...
  const lockedEnd = columns.filter((column) => column.locked && column.key !== "select");
...
function pinColumnAfter(columns, columnKey, afterKey, storageKey) {
  ...
  localStorage.setItem(storageKey, JSON.stringify(columns.filter((item) => !item.locked).map((item) => item.key)));
}
  ...
  setup(props) {
    return () =>
      h(
        ...
        [h("path", { d: ICON_PATHS[props.name] || ICON_PATHS.list })]
      );
  }
};
...
function dispatchQuickDateFilterForDate(dateValue) {
  const matched = DISPATCH_DATE_FILTERS.find((item) =>
    Number.isFinite(item.offset) && dateValue === offsetDateInputValue(item.offset)
  );
  return matched?.key || "custom";
}

function normalizeDispatchDateFilterKey(filterKey, fallbackDate = "") {
  if (DISPATCH_DATE_FILTERS.some((item) => item.key === filterKey)) return filterKey;
  return dispatchQuickDateFilterForDate(fallbackDate) || "today";
}

function dispatchDateFilterBounds(filterKey = dispatchDateFilter.value) {
  const today = parseInputDate(todayInputValue()) || new Date();
  const quickFilter = DISPATCH_DATE_FILTERS.find((item) => item.key === filterKey && Number.isFinite(item.offset));
  if (quickFilter) {
    const date = parseInputDate(todayInputValue()) || new Date();
    date.setDate(date.getDate() + quickFilter.offset);
    const value = dateInputFromDate(date);
  ...
  if (filterKey === "custom") {
    return {
      start: dispatchCustomDateStart.value || dispatchDate.value,
      end: dispatchCustomDateEnd.value || dispatchCustomDateStart.value || dispatchDate.value
    };
  }
  return { start: dispatchDate.value, end: dispatchDate.value };
}
...
const financeDateFilter = ref(localStorage.getItem("hanye_finance_date_filter") || "month");
const financeCustomDateStart = ref(localStorage.getItem("hanye_finance_custom_date_start") || "");
const financeCustomDateEnd = ref(localStorage.getItem("hanye_finance_custom_date_end") || "");
...
const statementExchangeRate = ref(localStorage.getItem("hanye_statement_exchange_rate") || "0.92");
const statementDownloadRows = ref(loadStoredJson("hanye_statement_download_rows", []));
const driverRouteAdjustRules = ref(loadStoredJson("hanye_driver_route_adjust_rules", []));
...
const storedDispatchDate = localStorage.getItem("hanye_dispatch_date") || offsetDateInputValue(1);
const dispatchDate = ref(storedDispatchDate);
const dispatchDateFilter = ref(normalizeDispatchDateFilterKey(localStorage.getItem("hanye_dispatch_date_filter"), storedDispatchDate));
const dispatchCustomDateStart = ref(localStorage.getItem("hanye_dispatch_custom_date_start") || "");
const dispatchCustomDateEnd = ref(localStorage.getItem("hanye_dispatch_custom_date_end") || "");
const activeDispatchStatusPool = ref(DISPATCH_PLAN_DEFAULT_STATUS);
const dispatchPlanRows = ref([]);
const dispatchLoadedDates = ref([storedDispatchDate]);
...
const homePendingOrders = computed(() =>
  orderRows.value.filter((order) => ["待确认", "待审核", "费用待确认", "缺票据"].includes(order.status))
);
...
function dispatchStorageKey(date = dispatchDate.value) {
  return `hanye_dispatch_plan_${date || todayInputValue()}`;
}
...
function localDispatchPlanRows(date = dispatchDate.value) {
  try {
    const rows = JSON.parse(localStorage.getItem(dispatchStorageKey(date)) || "[]");
...
async function persistDispatchPlanRows(date = dispatchDate.value, rows = dispatchPlanRows.value) {
  localStorage.setItem(dispatchStorageKey(date), JSON.stringify(rows));
...
async function readDispatchPlanRowsForDate(date = dispatchDate.value) {
  try {
    const result = await apiFetch(`/dispatch-plans/${encodeURIComponent(date)}`);
    const rows = Array.isArray(result.rows) ? result.rows : [];
    const localRows = localDispatchPlanRows(date);
    return rows.length === 0 && localRows.length > 0 ? localRows : rows;
  } catch {
    return localDispatchPlanRows(date);
  }
}
...
async function loadDispatchPlan(date = dispatchDate.value) {
  try {
    ...
    if (rows.length === 0 && localRows.length > 0) {
      rows = normalizeDispatchPlanRows(localRows, date);
      await persistDispatchPlanRows(date, rows);
      notify("已同步本地排车计划到服务器");
    }
    applyDispatchPlanRows(rows, date, [date]);
    localStorage.setItem(dispatchStorageKey(date), JSON.stringify(dispatchPlanRows.value));
  } catch (error) {
    applyDispatchPlanRows(localDispatchPlanRows(date), date, [date]);
    if (dispatchPlanRows.value.length === 0) selectedDispatchPlanIds.value = [];
    notify(error.message || "读取排车计划失败，已使用本地缓存");
  }
}

async function loadDispatchPlansForCurrentFilter() {
  const { start, end } = dispatchDateFilterBounds();
  const dates = datesBetweenInputValues(start, end);
  if (dates.length === 1) {
    await loadDispatchPlan(dates[0]);
    return;
  }
  try {
    const rowsByDate = await Promise.all(
      dates.map(async (date) => ({
        date,
        rows: await readDispatchPlanRowsForDate(date)
      }))
    );
    const rows = rowsByDate.flatMap(({ date, rows }) =>
      normalizeDispatchPlanRows(rows, date)
    );
    applyDispatchPlanRows(rows, dispatchDate.value, dates);
  } catch (error) {
    notify(error.message || "读取日期范围排车计划失败");
  }
}
...
function saveDispatchPlan({ silent = false } = {}) {
  ...
  Promise.all(saveJobs).catch((error) => {
...
async function syncDispatchRowOrderStatus(row, status) {
  ...
  const item = await apiFetch(`/orders/${encodeURIComponent(orderNo)}/status`, {
    method: "PATCH",
...
function generateDispatchNo(date = dispatchDate.value, extraRows = []) {
  ...
  for (let index = 0; index < localStorage.length; index += 1) {
    ...
    if (!key.startsWith("hanye_dispatch_plan_")) continue;
    try {
      ...
      // Ignore malformed historical local records.
    }
  }
...
async function saveEditedDispatchPlanRow() {
  ...
  try {
    ...
    if (updatedRow.orderNo) {
      ...
      if (currentOrder) {
        const item = await apiFetch(`/orders/${encodeURIComponent(updatedRow.orderNo)}`, {
          method: "PATCH",
          body: JSON.stringify({
            ...currentOrder,
...
async function saveManualDispatchPlanRow() {
  ...
  try {
    loading.value = true;
    const item = await apiFetch("/orders", {
      method: "POST",
      body: JSON.stringify({
...
function createDispatchPlanRow(order) {
  ...
  return {
    ...
    transportMode: mode,
    driver: order.driver || order.hkDriver || "",
...
function setDispatchDateByOffset(offset) {
  const matchedFilter = DISPATCH_DATE_FILTERS.find((item) => item.offset === offset);
  if (matchedFilter) dispatchDateFilter.value = matchedFilter.key;
  dispatchDate.value = offsetDateInputValue(offset);
  loadDispatchPlan();
}

function setDispatchDateFilter(filterKey) {
  dispatchDateFilter.value = normalizeDispatchDateFilterKey(filterKey, dispatchDate.value);
  const quickFilter = DISPATCH_DATE_FILTERS.find((item) => item.key === dispatchDateFilter.value && Number.isFinite(item.offset));
  if (quickFilter) setDispatchDateByOffset(quickFilter.offset);
}
...
async function applyDispatchPlanToOrders() {
  ...
  try {
    ...
    for (const row of dispatchPlanRows.value) {
      ...
      const item = await apiFetch(`/orders/${encodeURIComponent(order.no)}`, {
        method: "PATCH",
...
function isFinanceDateMatch(dateValue) {
  if (financeDateFilter.value === "custom") {
    const date = parseInputDate(dateValue);
    if (!date) return false;
    const start = parseInputDate(financeCustomDateStart.value);
    const end = parseInputDate(financeCustomDateEnd.value);
    if (start && date < start) return false;
    if (end && date > end) return false;
    return Boolean(start || end);
  }
  return isRelatedOrderDateMatch(dateValue, financeDateFilter.value);
}
...
function financeDateRangeLabel() {
  if (financeDateFilter.value === "custom") {
    const start = financeCustomDateStart.value || "未设开始";
    const end = financeCustomDateEnd.value || "未设结束";
    return `${start} 至 ${end}`;
  }
  return FINANCE_DATE_FILTERS.find((item) => item.key === financeDateFilter.value)?.label || "全部";
}

function setFinanceDateFilter(value) {
  financeDateFilter.value = FINANCE_DATE_FILTERS.some((item) => item.key === value) ? value : "month";
  localStorage.setItem("hanye_finance_date_filter", financeDateFilter.value);
}

function saveFinanceCustomDateRange() {
  localStorage.setItem("hanye_finance_custom_date_start", financeCustomDateStart.value);
  localStorage.setItem("hanye_finance_custom_date_end", financeCustomDateEnd.value);
}
...
const financeWageRows = computed(() =>
  sortRowsByTable(driverRows.value.map((driver) => {
    const orders = financeOrderRows.value.filter((order) =>
    ...
    const adjustmentBreakdown = financeAdjustmentRows.value
...
const financeSummary = computed(() => {
  const receivableHKD = financeOrderRows.value.reduce((sum, order) => sum + Number(order.receivableHKD || 0), 0);
  const receivableRMB = financeOrderRows.value.reduce((sum, order) => sum + Number(order.receivableRMB || 0), 0);
  const driverPayableHKD = financeWageRows.value.reduce((sum, row) => sum + row.total, 0);
  const driverPayableRMB = financeWageRows.value.reduce((sum, row) => sum + row.totalRMB, 0);
  const driverAdvanceHKD = financeWageRows.value.reduce((sum, row) => sum + row.advanceFee, 0);
  const driverAdvanceRMB = financeWageRows.value.reduce((sum, row) => sum + row.advanceFeeRMB, 0);
  const supplierAdvanceHKD = financeOrderRows.value
    .filter(orderAdvanceFeeBelongsToSupplier)
    .reduce((sum, order) => sum + orderAdvanceFeeHKD(order), 0);
  const supplierAdvanceRMB = financeOrderRows.value
    .filter(orderAdvanceFeeBelongsToSupplier)
    .reduce((sum, order) => sum + orderAdvanceFeeRMB(order), 0);
  const outsourcedHKD = financeOrderRows.value.reduce((sum, order) => sum + outsourcedCostAmountForOrder(order), 0);
  const supplierPayableHKD = financeOrderRows.value
    .filter(orderAdvanceFeeBelongsToSupplier)
    .reduce((sum, order) => {
      const payable = supplierOrderPayableBreakdown(order);
      return sum + payable.payableHKD + (payable.rule ? 0 : orderAdvanceFeeHKD(order));
    }, 0);
  const supplierPayableRMB = financeOrderRows.value
    .filter(orderAdvanceFeeBelongsToSupplier)
    .reduce((sum, order) => {
      const payable = supplierOrderPayableBreakdown(order);
  ...
  return {
    orderCount: financeOrderRows.value.length,
    receivableHKD,
    receivableRMB,
    driverPayableHKD,
    driverPayableRMB,
    ...
    supplierPayableHKD,
    supplierPayableRMB,
    outsourcedOrderCount: financeOrderRows.value.filter((order) => order.vehicleSource === "外派车辆").length
  };
});

function financeDateRangeBounds(filterKey = financeDateFilter.value) {
  const today = parseInputDate(todayInputValue()) || new Date();
  if (filterKey === "custom") {
    return {
      start: financeCustomDateStart.value || "",
      end: financeCustomDateEnd.value || ""
    };
  }
  if (filterKey === "lastMonth") {
  ...
  if (filterKey === "pastYears") {
    return { start: "", end: `${today.getFullYear() - 1}-12-31` };
  }
  return { start: "", end: "" };
}
...
const financeCustomerRanking = computed(() => {
  const groups = new Map();
  financeOrderRows.value.forEach((order) => {
    const key = order.customer || "未填写客户";
    const row = groups.get(key) || { customer: key, count: 0, hkd: 0, rmb: 0 };
    row.count += 1;
    row.hkd += Number(order.receivableHKD || 0);
    row.rmb += Number(order.receivableRMB || 0);
    groups.set(key, row);
  });
  return Array.from(groups.values()).sort((a, b) => b.count - a.count || b.hkd - a.hkd).slice(0, 12);
});
...
function markStatementDownloaded(type, entityName, start, end) {
  const key = statementDownloadKey(type, entityName, start, end);
  const nextRows = statementDownloadRows.value.filter((item) => item.key !== key);
  nextRows.unshift({
    key,
    type,
  ...
  statementDownloadRows.value = nextRows.slice(0, 300);
  saveStoredJson("hanye_statement_download_rows", statementDownloadRows.value);
}

const statementEntityOptions = computed(() => {
  if (statementExportType.value === "driver") {
    return driverRows.value.map((driver) => ({ id: driver.id, name: driver.name })).filter((item) => item.name);
  }
  if (statementExportType.value === "supplier") {
    const names = new Set([
      ...customerRows.value.filter((item) => item.type === "供应商").map((item) => item.name),
      ...orderRows.value.map((order) => order.supplier).filter(Boolean)
    ]);
...
function statementDateRange() {
  const fallback = financeDateRangeBounds(financeDateFilter.value);
  return {
...
function selectedStatementOrders() {
  ...
  return orderRows.value.filter((order) => {
    if (!isOrderVisibleInOrderManagement(order)) return false;
...
function saveDriverRouteAdjustRules() {
  saveStoredJson("hanye_driver_route_adjust_rules", driverRouteAdjustRules.value);
}
...
function addDriverRouteAdjustRule() {
  ...
  driverRouteAdjustRules.value = [
    {
      id: `route-adjust-${Date.now()}`,
...
function removeDriverRouteAdjustRule(rule) {
...
function isOrderDateFilterMatch(dateValue) {
  if (orderDateFilter.value === "all") return true;
  if (orderDateFilter.value === "custom") {
    const date = parseInputDate(dateValue);
    if (!date) return false;
    const start = parseInputDate(orderCustomDateStart.value);
    const end = parseInputDate(orderCustomDateEnd.value);
    if (start && date < start) return false;
    if (end && date > end) return false;
    return Boolean(start || end);
  }
  if (orderDateFilter.value === "yesterday") {
    const date = parseInputDate(dateValue);
    const today = parseInputDate(todayInputValue());
  ...
  return isRelatedOrderDateMatch(dateValue, orderDateFilter.value);
}
...
function isOrderColumnFrozen(column) {
  return (orderLockedColumns.value || []).includes(column?.key);
}
...
const visibleDispatchTableColumns = computed(() =>
  dispatchTableColumns.filter((column) => dispatchTableColumnVisibility[column.key] !== false)
);
...
const selectedFreightRate = computed(() =>
  freightRateRows.value.find((item) => item.id === selectedFreightRateId.value) || freightRateRows.value[0]
);
...
const freightRateGroups = computed(() => {
  const groups = new Map();
  freightRateRows.value.forEach((item) => {
    const normalized = normalizeFreightRateEntry(item);
  ...
  return [...groups.values()].sort((a, b) =>
    `${a.direction}${a.level1}${a.level2}${a.level3}`.localeCompare(`${b.direction}${b.level1}${b.level2}${b.level3}`, "zh-Hans-CN")
  );
});

const freightGroupsByKey = computed(() => new Map(freightRateGroups.value.map((group) => [group.key, group])));
...
function saveCurrentColumnOrder(columns, storageKey) {
  localStorage.setItem(storageKey, JSON.stringify(columns.filter((column) => !column.locked).map((column) => column.key)));
}
...
function isOrderColumnLocked(column) {
  return isManagedColumnLocked(orderLockedColumns, column);
}
...
function resetManagedColumnOrder(columns, lockedRef, orderStorageKey, lockedStorageKey) {
  ...
  const lockedStart = columns.filter((column) => column.locked && column.key !== "actions");
  const lockedEnd = columns.filter((column) => column.locked && column.key === "actions");
  const unlocked = columns.filter((column) => !column.locked).sort((left, right) =>
    (left.defaultIndex ?? 0) - (right.defaultIndex ?? 0)
  );
  columns.splice(0, columns.length, ...lockedStart, ...unlocked, ...lockedEnd);
  notify("已恢复默认列顺序");
...
async function syncAddressBookEntryToCustomerContact(row, contactId = "") {
  const customerId = orderForm.customerId || selectedCustomer.value?.id || "";
  const name = String(row.contact || "").trim();
  if (!customerId || !name) return false;
  ...
  const saved = await apiFetch(
    existing ? `/customer-contacts/${encodeURIComponent(existing.id)}` : "/customer-contacts",
    {
...
async function saveAddressBookEntry() {
  ...
  try {
    const contactSynced = await syncAddressBookEntryToCustomerContact(row, editingAddressBookId.value);
    resetAddressBookForm();
    addressBookFormOpen.value = false;
    notify(contactSynced ? "地址已保存到客户联系人" : "请先选择客户并填写联系人");
  } catch (error) {
...
async function deleteAddressBookEntry(id) {
  if (!id) return;
  try {
    await apiFetch(`/customer-contacts/${encodeURIComponent(id)}`, { method: "DELETE" });
...
async function deleteSelectedAddressBookEntries() {
  ...
  try {
    await Promise.all([...selected].map((id) =>
      apiFetch(`/customer-contacts/${encodeURIComponent(id)}`, { method: "DELETE" })
    ));
...
async function hideHistoricalAddress(option) {
  ...
  try {
    const hidden = await apiFetch("/address-history-hidden", {
      method: "POST",
...
async function apiFetchList(path, label, options = {}) {
  try {
    const data = await apiFetch(path);
    return Array.isArray(data) ? data : [];
  } catch (error) {
    console.warn(`${label || path} 加载失败`, error);
    if (!options.silent) {
      notify(`${label || path} 加载失败：${error.message}`);
    }
    return [];
  }
}
...
async function reloadTemplateRows(options = {}) {
  templateRows.value = await apiFetchList("/templates?includeContent=0&scope=export", "模板中心", options);
...
function openStoredFile(file, action = "preview") {
  ...
  window.open(fileEndpoint(file, action), "_blank", "noopener");
}
...
const SAFE_UPLOAD_EXTENSIONS = new Set(["png", "jpg", "jpeg", "webp", "gif", "pdf", "txt", "csv", "doc", "docx", "xls", "xlsx"]);
const MAX_UPLOAD_BYTES = 8 * 1024 * 1024;

function uploadFileValidationMessage(file) {
  const extension = String(file?.name || "").split(".").pop()?.toLowerCase() || "";
  if (!SAFE_UPLOAD_EXTENSIONS.has(extension)) return "仅支持图片、PDF、Word、Excel、CSV 或 TXT 文件";
...
async function loadFiles(entityType, entityId) {
  ...
  return apiFetch(`/files?entityType=${encodeURIComponent(entityType)}&entityId=${encodeURIComponent(entityId)}`);
}

async function loadDeletedFiles(entityType, entityId) {
  if (!entityType || !entityId) return [];
  return apiFetch(`/files?entityType=${encodeURIComponent(entityType)}&entityId=${encodeURIComponent(entityId)}&deletedOnly=1`);
}

async function uploadFileFor(entityType, entityId, category, event, targetRows) {
...
async function restoreFile(file) {
  try {
    const restored = await apiFetch(`/files/${file.id}/restore`, { method: "POST" });
...
async function purgeFile(file) {
  ...
  try {
    await apiFetch(`/files/${file.id}/permanent`, { method: "DELETE" });
    attachmentRecycleRows.value = attachmentRecycleRows.value.filter((item) => item.id !== file.id);
    notify("附件已彻底删除");
  } catch (error) {
    notify(error.message);
...
async function loadDatabaseData(options = {}) {
  ...
  try {
    const [
      ...
      apiFetchList("/customers", "客户/供应商"),
      apiFetchList("/customer-contacts", "联系人"),
      apiFetchList("/orders", "订单"),
      apiFetchList("/vehicles", "车辆"),
      apiFetchList("/drivers", "司机"),
      apiFetchList("/driver-wage-rules", "司机费用规则"),
    ...
    editRule(ruleData.find((item) => item.id === selectedRuleId.value) || ruleData[0] || null);
...
function openModule(id) {
  ...
  const hashTarget = next === "vehicleDriver" ? "vehicleManage" : next;
...
function openCustomerModal(customer = null) {
  ...
  Object.assign(customerForm, {
    type: customer?.type || activePartnerType.value,
    name: customer?.name || "",
...
async function saveCustomer() {
  try {
    loading.value = true;
    const method = editingCustomerId.value ? "PATCH" : "POST";
    const path = editingCustomerId.value ? `/customers/${encodeURIComponent(editingCustomerId.value)}` : "/customers";
    const item = await apiFetch(path, { method, body: JSON.stringify(buildCustomerPayload()) });
...
async function saveOrder() {
  try {
    ...
    const method = editingOrderNo.value ? "PATCH" : "POST";
    const path = editingOrderNo.value ? `/orders/${encodeURIComponent(editingOrderNo.value)}` : "/orders";
...
function uploadOrderFile(event) {
  uploadFileFor("order", editingOrderNo.value, "订单附件", event, orderAttachmentRows);
}
...
function uploadOrderFeeFile(fee, index, event) {
  ...
  uploadFileFor("order", editingOrderNo.value, feeAttachmentCategory(fee, index), event, orderAttachmentRows);
}

async function loadCustomerFiles() {
  customerFileRows.value = selectedCustomer.value
    ? await apiFetch(`/files?entityType=customer&entityId=${encodeURIComponent(selectedCustomer.value.id)}&includeOrderFiles=1`)
    : [];
}
...
async function dispatchRowsForOrder(order = {}) {
  const orderNo = order.no || "";
  if (!orderNo) return [];
  const loadedRows = dispatchPlanRows.value.filter((row) => row.orderNo === orderNo);
  if (loadedRows.length) return loadedRows;
  if (!order.date) return [];
  const rows = await readDispatchPlanRowsForDate(order.date);
  return normalizeDispatchPlanRows(rows, order.date).filter((row) => row.orderNo === orderNo);
}

async function orderDispatchReachedAuditStatus(order = {}) {
  const rows = await dispatchRowsForOrder(order);
  return rows.some((row) => normalizeDispatchPlanStatus(row.status, row) === "已签收");
}

async function confirmForceAuditIfNeeded(orders = []) {
  const targets = orders.filter((order) => order.status !== "已审核");
  if (!targets.length) return true;
  const notSigned = [];
  for (const order of targets) {
    if (!(await orderDispatchReachedAuditStatus(order))) notSigned.push(order);
  }
  const abnormalCount = targets.filter((item) => ["缺票据", "费用待确认"].includes(item.status)).length;
  const messages = [];
  if (notSigned.length) {
    messages.push(`有 ${notSigned.length} 条订单未达到已签收状态，暂未达到审核状态。`);
  }
  if (abnormalCount) {
    messages.push(`有 ${abnormalCount} 条订单可能有单据未上传或费用未确认。`);
  }
...
async function auditOrder(order) {
  ...
  if (!(await confirmForceAuditIfNeeded([order]))) return;
  try {
    const updated = await apiFetch(`/orders/${encodeURIComponent(order.no)}/status`, {
      method: "PATCH",
      body: JSON.stringify({ status: "已审核" })
    });
...
async function auditPendingOrders() {
  ...
  const auditableOrders = scope.filter((item) => item.status !== "已审核");
  if (!(await confirmForceAuditIfNeeded(auditableOrders))) return;
  ...
  try {
    const updatedOrders = await apiFetch("/orders/audit", {
      method: "POST",
      body: JSON.stringify({ orderNos: pendingNos })
    });
...
async function cancelSelectedAudits() {
  ...
  try {
    const updatedOrders = await Promise.all(auditedOrders.map((order) =>
      apiFetch(`/orders/${encodeURIComponent(order.no)}/status`, {
        method: "PATCH",
        body: JSON.stringify({ status: "待审核" })
      })
    ));
...
async function deleteSelectedOrders() {
  ...
  try {
    for (const order of targets) {
      await apiFetch(`/orders/${encodeURIComponent(order.no)}`, { method: "DELETE" });
    }
...
async function auditSelectedCustomerOrders() {
  ...
  const auditableOrders = targets.filter((item) => item.status !== "已审核");
  if (!(await confirmForceAuditIfNeeded(auditableOrders))) return;
  ...
  try {
    const updatedOrders = await apiFetch("/orders/audit", {
      method: "POST",
      body: JSON.stringify({ orderNos: auditableOrders.map((item) => item.no) })
    });
    const updatedMap = new Map(updatedOrders.map((item) => [item.no, item]));
    orderRows.value = orderRows.value.map((item) => updatedMap.get(item.no) || item);
...
async function openRecycleBin() {
  try {
    recycleRows.value = await apiFetch("/orders/recycle");
...
async function restoreOrder(order) {
  try {
    const restored = await apiFetch(`/orders/${encodeURIComponent(order.no)}/restore`, { method: "POST" });
...
async function exportOrderRowsAsCsv(orders, title = "订单导出", templateRow = selectedTemplate.value, exchangeOverride = null) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(message || "Excel 导出失败");
    }
...
async function exportOrderRowsAsPdf(orders, title = "订单导出", templateRow = selectedTemplate.value, exchangeOverride = null) {
  ...
  try {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      throw new Error(error.message || "PDF 导出失败");
    }
    const blob = await response.blob();
...
async function saveVehicle() {
  try {
    loading.value = true;
    const method = editingVehiclePlate.value ? "PATCH" : "POST";
    const path = editingVehiclePlate.value ? `/vehicles/${encodeURIComponent(editingVehiclePlate.value)}` : "/vehicles";
    const item = await apiFetch(path, { method, body: JSON.stringify(vehicleForm) });
...
async function saveDriver() {
  try {
    ...
    loading.value = true;
    const method = editingDriverId.value ? "PATCH" : "POST";
    const path = editingDriverId.value ? `/drivers/${editingDriverId.value}` : "/drivers";
    const item = await apiFetch(path, { method, body: JSON.stringify(driverForm) });
...
async function saveDriverWageRule() {
  try {
    ...
    const path = payload.id ? `/driver-wage-rules/${payload.id}` : "/driver-wage-rules";
    const item = await apiFetch(path, { method, body: JSON.stringify(payload) });
...
async function deleteDriverWageRule(item) {
  ...
  try {
    await apiFetch(`/driver-wage-rules/${item.id}`, { method: "DELETE" });
...
async function upsertSupplierCostRuleContent(content, currentId = null) {
  ...
  let target = existing;
  try {
    item = await apiFetch(target ? `/templates/${target.id}` : "/templates", {
    ...
    item = await apiFetch(`/templates/${target.id}`, {
      method: "PATCH",
...
async function deleteSupplierCostRule(rule) {
  ...
  try {
    await apiFetch(`/templates/${rule.id}`, { method: "DELETE" });
...
async function saveDriverAdjustment() {
  try {
    ...
    const method = payload.id ? "PATCH" : "POST";
    const path = payload.id ? `/driver-adjustments/${payload.id}` : "/driver-adjustments";
    const item = await apiFetch(path, { method, body: JSON.stringify(payload) });
...
async function deleteDriverAdjustment(item) {
  ...
  try {
    await apiFetch(`/driver-adjustments/${item.id}`, { method: "DELETE" });
...
async function exportStatementCsv() {
  ...
  try {
    ...
    if (statementExportType.value === "supplier") {
      ...
      markStatementDownloaded("supplier", entityName, start, end);
    ...
    if (statementExportType.value === "driver") {
      ...
      markStatementDownloaded("driver", entityName, start, end);
      notify("已导出司机对账单");
      return;
    }
    ...
    markStatementDownloaded("customer", entityName, start, end);
...
async function exportStatementByFormat(format, templateRow = selectedTemplate.value) {
  ...
  if (ok) markStatementDownloaded(statementExportType.value, entityName, start, end);
}
...
async function ensureTemplateContent(item = selectedTemplate.value) {
  ...
  const fullItem = await apiFetch(`/templates/${item.id}`);
...
function exportCustomers() {
  ...
  exportCsv(
    ...
    ["编号", "类型", "名称", "城市", "账期", "应收人民币", "应收港币", "最近订单日期", "创建日期"],
    rows.map((item) => [item.id, item.type, item.name, item.city, item.term, item.receivableRMB, item.receivableHKD, partnerRecentOrderDate(item), item.createdAt])
  );
...
async function deleteSelectedCustomer() {
  ...
  try {
    for (const target of targets) {
      await apiFetch(`/customers/${encodeURIComponent(target.id)}`, { method: "DELETE" });
    }
...
async function deleteSelectedVehicleDriver() {
  try {
    if (activeVehicleTab.value === "车辆管理") {
      ...
      for (const target of targets) {
        await apiFetch(`/vehicles/${encodeURIComponent(target.plate)}`, { method: "DELETE" });
      }
    ...
    if (activeVehicleTab.value === "司机管理") {
      ...
      for (const target of targets) {
        await apiFetch(`/drivers/${target.id}`, { method: "DELETE" });
      }
      const deletedIds = new Set(targets.map((item) => item.id));
...
async function saveFeeItemRow(item) {
  try {
    const saved = await apiFetch(`/fee-items/${item.id}`, {
      method: "PATCH",
      body: JSON.stringify({ ...feeItemRowDraft, id: item.id })
    });
...
async function saveFeeItem() {
  try {
    const method = feeItemForm.id ? "PATCH" : "POST";
    const path = feeItemForm.id ? `/fee-items/${feeItemForm.id}` : "/fee-items";
    const item = await apiFetch(path, { method, body: JSON.stringify(feeItemForm) });
...
async function deleteFeeItem(item) {
  try {
    await apiFetch(`/fee-items/${item.id}`, { method: "DELETE" });
...
async function persistFeeItemOrder(items, options = {}) {
  ...
  try {
    const saved = await apiFetch("/fee-items/order", {
      method: "PATCH",
      body: JSON.stringify({ ids: orderedItems.map((item) => item.id) })
    });
...
async function upsertFreightRateForGroup(group, tonnage, amountPatch) {
  ...
  const path = current?.id ? `/freight-rates/${current.id}` : "/freight-rates";
...
async function saveFreightRate() {
  try {
    ...
    const method = freightRateForm.id ? "PATCH" : "POST";
    const path = freightRateForm.id ? `/freight-rates/${freightRateForm.id}` : "/freight-rates";
    const item = await apiFetch(path, { method, body: JSON.stringify(freightRateForm) });
...
async function deleteFreightRate(item) {
  ...
  try {
    await apiFetch(`/freight-rates/${item.id}`, { method: "DELETE" });
...
async function deleteFreightGroup(group) {
  ...
  try {
    await Promise.all(items.map((item) => apiFetch(`/freight-rates/${item.id}`, { method: "DELETE" })));
...
async function deleteSavedFeeTemplate(item) {
  ...
  try {
    await apiFetch(`/templates/${item.id}`, { method: "DELETE" });
...
async function saveCurrentFeesAsTemplate() {
  ...
  try {
    ...
    const item = await apiFetch(existing ? `/templates/${existing.id}` : "/templates", {
      method: existing ? "PATCH" : "POST",
      body: JSON.stringify(existing ? { ...existing, ...payload } : payload)
    });
...
async function saveTemplate() {
  ...
  try {
    ...
    const method = payload.id ? "PATCH" : "POST";
    const path = payload.id ? `/templates/${payload.id}` : "/templates";
    const item = await apiFetch(path, { method, body: JSON.stringify(payload) });
...
async function autosaveTemplateDesignerContent(options = {}) {
  ...
  try {
    ...
    const item = await apiFetch(`/templates/${payload.id}`, {
      method: "PATCH",
      body: JSON.stringify(payload)
    });
    templateRows.value = templateRows.value.map((row) => row.id === item.id ? item : row);
...
async function duplicateTemplate(item) {
  try {
    ...
    const copied = await apiFetch("/templates", { method: "POST", body: JSON.stringify(payload) });
...
async function saveRule() {
  try {
    ...
    const path = ruleForm.id ? `/rules/${ruleForm.id}` : "/rules";
    const item = await apiFetch(path, { method, body: JSON.stringify(ruleForm) });
...
async function deleteRule(item) {
  try {
    await apiFetch(`/rules/${item.id}`, { method: "DELETE" });
...
async function saveMaster() {
  try {
    const method = masterForm.id ? "PATCH" : "POST";
    const path = masterForm.id ? `/master-data/${masterForm.id}` : "/master-data";
    const item = await apiFetch(path, { method, body: JSON.stringify(masterForm) });
...
async function deleteMaster(item) {
  try {
    await apiFetch(`/master-data/${item.id}`, { method: "DELETE" });
...
async function refreshAuditLogs() {
  try {
    auditRows.value = await apiFetch("/audit-logs");
    notify("审计日志已刷新");
  } catch (error) {
    notify(error.message);
  }
...
async function saveNewContactRow() {
  ...
  try {
    const saved = await apiFetch("/customer-contacts", {
      method: "POST",
      body: JSON.stringify({
        ...contactRowDraft,
...
async function saveContactRow(contact) {
  try {
    ...
    const saved = await apiFetch(`/customer-contacts/${encodeURIComponent(contact.id)}`, {
      method: "PATCH",
      body: JSON.stringify(payload)
    });
...
async function saveContact() {
  try {
    ...
    const saved = await apiFetch(
      editingContactId.value ? `/customer-contacts/${encodeURIComponent(editingContactId.value)}` : "/customer-contacts",
      {
...
async function deleteContact(contact) {
  try {
    await apiFetch(`/customer-contacts/${encodeURIComponent(contact.id)}`, { method: "DELETE" });
...
function syncVehicleDriverTabFromRoute(routeKey) {
  if (routeKey === "vehicleManage" || routeKey === "vehicle") {
    activeVehicleTab.value = "车辆管理";
...
watch([activeVehicleTab, activeDriverDetailTab, selectedDriverId], () => {
  ...
  if (activeVehicleTab.value === "司机管理" && activeDriverDetailTab.value === "司机费用规则") {
    editDriverWageRule(selectedDriverWageRules.value[0] || null);
  }
...
watch(dispatchDate, () => {
  if (["yesterday", "today", "tomorrow"].includes(dispatchDateFilter.value)) {
    const nextFilter = dispatchQuickDateFilterForDate(dispatchDate.value);
    dispatchDateFilter.value = nextFilter;
    if (nextFilter === "custom") {
      dispatchCustomDateStart.value = dispatchDate.value;
      dispatchCustomDateEnd.value = dispatchDate.value;
    }
  }
  localStorage.setItem("hanye_dispatch_date", dispatchDate.value);
...
watch(dispatchDateFilter, () => {
  localStorage.setItem("hanye_dispatch_date_filter", dispatchDateFilter.value);
  loadDispatchPlansForCurrentFilter();
});

watch([dispatchCustomDateStart, dispatchCustomDateEnd], () => {
    ...
    <main class="main-panel">
      <section v-if="activeModule === 'home'" class="work-page home-page">
        ...
        <div class="home-kpi-grid">
          <button class="home-kpi-card" type="button" @click="openModule('customers')">
            <span>客户总数</span>
        ...
        <div class="home-board-grid">
          ...
          <section class="table-card home-board-card">
            <div class="home-board-head">
              ...
              <button class="ghost-btn small" type="button" @click="openModule('customers')">客户管理</button>
            </div>
            <div class="home-customer-list">
              <button
                ...
                @click="activePartnerType = '客户'; selectedCustomerId = customer.id; openModule('customers')"
              >
      ...
      <section v-else-if="activeModule === 'customers'" class="work-page">
        <div class="toolbar" @click="orderExportMenuOpen = false">
          <div class="segmented">
            <button :class="{ active: activePartnerType === '客户' }" @click="activePartnerType = '客户'">客户</button>
            <button :class="{ active: activePartnerType === '供应商' }" @click="activePartnerType = '供应商'">供应商</button>
          </div>
          <input v-model.trim="partnerSearch" class="search-input" placeholder="公司名 / 税号 / 订单号 / 联系人 / 手机号" />
          ...
          <button class="primary-btn" :disabled="loading" @click="openCustomerModal()"><IconSvg name="plus" />新增{{ activePartnerType }}</button>
        </div>
                ...
                <tbody>
                  <tr v-for="order in selectedCustomerOrders" :key="order.no">
                    ...
                    >
                      ...
                      <button v-if="order.status !== '已审核'" class="icon-btn icon-only success" type="button" title="审核订单" aria-label="审核订单" @click="auditOrder(order)"><IconSvg name="check" /></button>
            ...
            <div v-else-if="activeCustomerDetailTab === '附件管理'" class="file-panel">
              <div class="file-toolbar">
                <label class="file-upload-btn">
                  上传附件
                  <input type="file" @change="uploadCustomerFile" />
                </label>
                <span class="hint">客户合同、资质、往来附件会保存到服务器文件库。</span>
              </div>
              <table class="data-table compact">
      ...
      <section v-else-if="activeModule === 'orders'" class="work-page order-page">
        <div class="toolbar order-page-toolbar">
          <div class="order-filter-group">
            ...
            <div v-if="orderDateFilter === 'custom'" class="order-custom-date-range">
              <input v-model="orderCustomDateStart" type="date" title="开始日期" />
              <span>至</span>
              <input v-model="orderCustomDateEnd" type="date" title="结束日期" />
            </div>
              ...
              <thead>
                <tr>
                  <th
                    ...
                    :key="column.key"
                    :class="['resizable-th', { sortable: !['select', 'actions'].includes(column.key), sorted: tableSortDirection('orders', column.key), 'sticky-managed-column': isOrderColumnFrozen(column) }]"
                    :style="orderFrozenColumnStyle(column, index)"
                  ...
                  >
                    ...
                    <span v-else-if="column.key === 'actions'" class="column-manager-wrap" @click.stop>
                      ...
                      <div v-if="orderColumnMenuOpen" class="column-manager-menu" @click.stop>
                        <div
                          v-for="menuColumn in orderColumns.filter((item) => !item.locked)"
                          :key="menuColumn.key"
                          class="column-manager-row"
                          draggable="true"
              ...
              <tbody>
                ...
                >
                  <td
                    v-for="(column, index) in visibleOrderColumns"
                    :key="column.key"
                    :class="{ 'row-actions': column.key === 'actions', 'sticky-managed-column': isOrderColumnFrozen(column) }"
                    :style="orderFrozenColumnStyle(column, index)"
                    :title="orderTableCellTitle(order, column.key)"
                  >
                    <input v-if="column.key === 'select'" v-model="selectedOrderNos" type="checkbox" :value="order.no" @click.stop />
                    <span v-else-if="column.key === 'status'" class="status-badge" :class="orderStatusClass(order.status)">{{ order.status }}</span>
                    <template v-else-if="column.key === 'actions'">
                      <button v-if="order.status !== '已审核'" class="icon-btn success" @click.stop="auditOrder(order)"><IconSvg name="check" />审核</button>
                      <button v-else class="icon-btn" @click.stop="cancelAuditOrder(order)"><IconSvg name="refresh" />取消审核</button>
                      <button class="icon-btn" @click.stop="openOrderModal(null, order)"><IconSvg name="edit" />编辑</button>
                      <button v-if="canDeleteOrder(order)" class="icon-btn danger" @click.stop="deleteOrder(order)"><IconSvg name="trash" />删除</button>
                    </template>
      ...
      <section v-else-if="activeModule === 'dispatchBoard'" class="work-page dispatch-page">
        <div class="toolbar dispatch-toolbar">
          ...
          <div class="dispatch-date-group">
            <div class="dispatch-range-control">
              <div class="segmented compact-segmented dispatch-date-segmented">
                <button
                  v-for="item in DISPATCH_DATE_FILTERS"
                  :key="item.key"
                  type="button"
                  :class="{ active: dispatchDateFilter === item.key }"
                  @click="setDispatchDateFilter(item.key)"
                >{{ item.label }}</button>
              </div>
            </div>
            <div v-if="dispatchDateFilter === 'custom'" class="dispatch-custom-date-range">
              <input v-model="dispatchCustomDateStart" type="date" title="开始日期" />
              <span>至</span>
              <input v-model="dispatchCustomDateEnd" type="date" title="结束日期" />
            </div>
          </div>
        ...
        <div class="dispatch-status-pool">
          <button
            ...
            :key="item.status"
            type="button"
            :class="{ active: activeDispatchStatusPool === item.status }"
                  ...
                  <tr v-for="row in searchedDispatchPlanRows" :key="row.id" @click="openDispatchDetail(row)">
                    ...
                    >
                      ...
                      <template v-else-if="column.key === 'status'">
                        <select v-model="dispatchPlanRows[row.index].status" class="dispatch-status-select" @click.stop @change="handleDispatchStatusChange(row)">
      ...
      <section v-else-if="activeModule === 'vehicleDriver'" class="work-page">
        <div class="toolbar">
          <div class="segmented">
            <button :class="{ active: activeVehicleTab === '车辆管理' }" @click="activeVehicleTab = '车辆管理'">车辆管理</button>
            <button :class="{ active: activeVehicleTab === '司机管理' }" @click="activeVehicleTab = '司机管理'">司机管理</button>
          </div>
          <input v-model.trim="vehicleDriverSearch" class="search-input" placeholder="车牌 / 司机 / 证件 / 备注" />
          ...
          <div class="detail-panel">
            <template v-if="activeVehicleTab === '车辆管理'">
              ...
              <div v-else-if="activeVehicleDetailTab === '证件提醒'" class="file-panel">
                ...
                <div class="file-toolbar">
                  <label class="file-upload-btn"><IconSvg name="upload" />上传证件/保险单<input type="file" @change="uploadVehicleFile" /></label>
              ...
              <table v-else-if="activeVehicleDetailTab === '费用记录'" class="data-table compact">
                <thead><tr><th>月份</th><th>类型</th><th>金额</th><th>备注</th></tr></thead>
                <tbody>
                  <tr><td>本月</td><td>车辆费用</td><td>HKD {{ money(selectedVehicle?.monthlyCost || 0) }}</td><td>{{ selectedVehicle?.note || '-' }}</td></tr>
                </tbody>
              </table>
            ...
            <template v-else>
              <div class="tabs">
                ...
                <button :class="{ active: activeDriverDetailTab === '司机费用规则' }" @click="activeDriverDetailTab = '司机费用规则'">司机费用规则</button>
              ...
              <div v-else-if="activeDriverDetailTab === '证件照片'" class="file-panel">
                <div class="file-toolbar">
                  <label class="file-upload-btn"><IconSvg name="upload" />上传司机证件<input type="file" @change="uploadDriverFile" /></label>
                  <span class="hint">驾驶证、身份证、港澳证等文件会保存到服务器文件库。</span>
                </div>
                <table class="data-table compact">
                  <thead><tr><th>文件名</th><th>大小</th><th>上传时间</th><th>操作</th></tr></thead>
                  <tbody>
              ...
              <div v-else-if="activeDriverDetailTab === '司机费用规则'" class="driver-wage-rules-panel">
                <div class="driver-rule-mode-strip">
                  <span>当前司机：{{ selectedDriver?.name || "-" }}</span>
                  <button
                    v-for="mode in driverWageTransportModeOptions"
                    :key="mode"
                    type="button"
                    :class="['ghost-btn small', { active: normalizeTransportMode(driverWageRuleForm.transportMode) === mode }]"
                    @click="prepareDriverWageRuleForMode(mode)"
                  >+ {{ mode }}规则</button>
                  <small>{{ selectedDriverIsMainlandRider ? '大陆骑师只匹配双司机规则。' : '司机费用按方向、城市、运输模式、币种匹配，可用进出口通用兜底。' }}</small>
                </div>
                <form class="driver-wage-rule-toolbar" @submit.prevent="saveDriverWageRule">
                  <label class="driver-rule-direction">方向
                    <select v-model="driverWageRuleForm.direction">
                      <option>{{ SHARED_DIRECTION }}</option>
                      <option v-for="item in DIRECTION_OPTIONS" :key="item">{{ item }}</option>
                    </select>
                  </label>
                  <label class="driver-rule-area">计价城市
                    <select v-model="driverWageRuleForm.city" required>
                      <option value="">请选择城市</option>
                      <option v-for="area in driverWageAreaOptions" :key="area" :value="area">{{ area }}</option>
                    </select>
                  </label>
                  <label>运输模式
                    <select v-model="driverWageRuleForm.transportMode">
                      <option v-for="mode in driverWageTransportModeOptions" :key="mode">{{ mode }}</option>
                    </select>
                  </label>
                  <label class="driver-rule-currency">币种
                    <select v-model="driverWageRuleForm.currency">
                      <option value="港币">HKD</option>
                      <option value="人民币">RMB</option>
                    </select>
                  </label>
                  <label>基础RMB<input v-model.number="driverWageRuleForm.baseRMB" type="number" min="0" step="0.01" /></label>
                  <label>基础HKD<input v-model.number="driverWageRuleForm.baseHKD" type="number" min="0" step="0.01" /></label>
                  <label>装货费 HKD<input v-model.number="driverWageRuleForm.loadPerBoard" type="number" min="0" step="0.01" /></label>
                  <label>卸货费 HKD<input v-model.number="driverWageRuleForm.unloadPerBoard" type="number" min="0" step="0.01" /></label>
                  <label>过海费 HKD<input v-model.number="driverWageRuleForm.crossSeaFee" type="number" min="0" step="0.01" /></label>
                  <label>加点费 HKD<input v-model.number="driverWageRuleForm.addPointFee" type="number" min="0" step="0.01" /></label>
                  <label>装货等候费 HKD<input v-model.number="driverWageRuleForm.waitingPerHour" type="number" min="0" step="0.01" /></label>
                  <label v-for="item in advanceFeeItemRows" :key="item.id">
                    {{ advanceFeeRateLabel(item) }}
                    <input :value="advanceFeeRateValue(driverWageRuleForm.advanceFeeRates, item)" type="number" min="0" step="0.01" @input="setAdvanceFeeRate(driverWageRuleForm.advanceFeeRates, item, $event.target.value)" />
                  </label>
                  <label class="driver-rule-note">说明<input v-model.trim="driverWageRuleForm.note" placeholder="例如：出口装货地深圳 HKD500/趟" /></label>
                  <div class="driver-rule-actions">
                    <button class="primary-btn small" type="submit"><IconSvg name="save" />保存规则</button>
                    <button class="ghost-btn small" type="button" @click="notify('请选择城市后逐条保存；批量填价稍后接入多选城市')"><IconSvg name="check" />批量填价</button>
                    <button class="ghost-btn small" type="button" @click="prepareNewDriverWageRule(false)">清空</button>
                  </div>
                </form>
                <div class="table-wrap">
                  <table class="data-table compact driver-wage-rule-table">
                    <thead>
                      <tr>
	                        <th>方向</th><th>计价城市</th><th>运输模式</th><th>基础RMB</th><th>基础HKD</th><th>装货费</th><th>卸货费</th><th>过海费</th><th>加点费</th><th>装货等候费</th><th v-for="item in advanceFeeItemRows" :key="item.id">{{ advanceFeeRateLabel(item) }}</th><th>说明</th><th>操作</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="rule in selectedDriverWageRules" :key="rule.id" :class="{ selected: selectedDriverWageRuleId === rule.id }">
                        <td>{{ rule.direction }}</td>
                        <td>{{ driverWageCityValue(rule.city) }}</td>
                        <td>{{ normalizeTransportMode(rule.transportMode || '单司机') }}</td>
                        <td>{{ money(rule.baseRMB) }}</td>
                        <td>{{ money(rule.baseHKD) }}</td>
                        <td>{{ money(rule.loadPerBoard) }}</td>
                        <td>{{ money(rule.unloadPerBoard) }}</td>
                        <td>{{ money(rule.crossSeaFee) }}</td>
                        <td>{{ money(rule.addPointFee) }}</td>
                        <td>{{ money(rule.waitingPerHour) }}</td>
	                        <td v-for="item in advanceFeeItemRows" :key="item.id">{{ money(advanceFeeRateValue(rule.advanceFeeRates, item)) }}</td>
                        <td>{{ rule.note || "-" }}</td>
                        <td class="row-actions">
                          <button class="icon-btn" @click="editDriverWageRule(rule)"><IconSvg name="edit" />编辑</button>
                          <button class="icon-btn danger" @click="deleteDriverWageRule(rule)"><IconSvg name="trash" />删除</button>
                        </td>
                      </tr>
	                      <tr v-if="selectedDriverWageRules.length === 0"><td :colspan="advanceFeeItemRows.length + 12">暂无司机费用规则</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
      ...
      <section v-else-if="activeModule === 'financeWages'" class="work-page finance-page finance-wage-page">
        ...
        <div class="finance-filter-bar">
          <div class="segmented finance-date-tabs">
            <button
              v-for="item in FINANCE_DATE_FILTERS"
              :key="item.key"
              type="button"
              :class="{ active: financeDateFilter === item.key }"
              @click="setFinanceDateFilter(item.key)"
            >{{ item.label }}</button>
          </div>
          <div v-if="financeDateFilter === 'custom'" class="finance-custom-range">
            <label>开始<input v-model="financeCustomDateStart" type="date" @change="saveFinanceCustomDateRange" /></label>
            <label>结束<input v-model="financeCustomDateEnd" type="date" @change="saveFinanceCustomDateRange" /></label>
          </div>
      ...
      <section v-else-if="activeModule === 'financeCosts'" class="work-page finance-page">
        ...
        <div class="finance-summary-grid">
          <div class="finance-summary-card"><span>订单数</span><strong>{{ financeSummary.orderCount }}</strong></div>
          <div class="finance-summary-card"><span>应收港币</span><strong>HKD {{ money(financeSummary.receivableHKD) }}</strong></div>
          <div class="finance-summary-card"><span>应收人民币</span><strong>RMB {{ money(financeSummary.receivableRMB) }}</strong></div>
          <div class="finance-summary-card"><span>外派成本</span><strong>{{ moneyPair(financeSummary.supplierPayableHKD, financeSummary.supplierPayableRMB) }}</strong></div>
        </div>
        <div class="finance-insight-grid">
          <div class="table-card finance-tool-card">
            <div class="table-card-head">
              <div>
                <strong>客户单量排名</strong>
                <span>{{ financeDateRangeLabel() }} · 按订单份数排序</span>
              </div>
            </div>
            <table class="data-table compact">
              <thead><tr><th>排名</th><th>客户</th><th>份数</th><th>港币</th><th>人民币</th></tr></thead>
              <tbody>
                <tr v-for="(row, index) in financeCustomerRanking" :key="row.customer">
                  <td>{{ index + 1 }}</td>
                  <td>{{ row.customer }}</td>
                  <td>{{ row.count }}</td>
                ...
                <tr v-if="financeCustomerRanking.length === 0"><td colspan="5">暂无客户单量</td></tr>
              </tbody>
            </table>
          </div>
        </div>
        <div class="finance-operations-grid" @click="statementExportMenuOpen = false">
          <div class="table-card finance-tool-card">
            <div class="table-card-head">
              <div>
                <strong>对账单导出</strong>
                <span>客户、司机、供应商都可按自定义时间段下载</span>
              </div>
              <span class="order-export-wrap statement-export-wrap" @click.stop>
                <button class="primary-btn small" type="button" @click="toggleStatementExportMenu">
                  <IconSvg name="download" />导出对账单<IconSvg name="chevronDown" />
                </button>
                <div v-if="statementExportMenuOpen" class="order-export-menu statement-export-menu">
                  <div class="order-export-menu-title">选择导出模板</div>
                  <div class="statement-export-menu-meta">
                    <span>{{ statementExportTypeLabel() }}</span>
                    <span>汇总：{{ statementSettlementCurrency }}</span>
                    <span>汇率：{{ statementExchangeRate || "-" }}</span>
                  </div>
                  <div v-for="template in orderExportTemplateOptions()" :key="template.id" class="order-export-template-row">
                    <span :title="template.name">{{ template.name }}</span>
            ...
            <div class="statement-export-grid">
              <label>类型
                <select v-model="statementExportType" @change="statementExportEntity = ''; saveStatementExportSettings()">
                  <option value="customer">客户对账单</option>
                  <option value="driver">司机对账单</option>
                  <option value="supplier">供应商对账单</option>
                </select>
              </label>
              <label>对象
                <select v-model="statementExportEntity">
                  <option v-for="item in statementEntityOptions" :key="item.id || item.name" :value="item.name">{{ item.name }}</option>
                </select>
              </label>
              <label>开始日期<input v-model="statementExportStart" type="date" @change="saveStatementExportSettings" /></label>
              <label>结束日期<input v-model="statementExportEnd" type="date" @change="saveStatementExportSettings" /></label>
              <label>汇总币种
                <select v-model="statementSettlementCurrency" @change="saveStatementExportSettings">
                  <option value="人民币">RMB</option>
                  <option value="港币">HKD</option>
                </select>
              </label>
              <label>汇率 HKD→RMB<input v-model="statementExchangeRate" type="number" min="0" step="0.0001" @change="saveStatementExportSettings" /></label>
            </div>
          </div>
        </div>
        <div class="table-card finance-table-card">
          <table class="data-table compact">
            ...
            <tbody>
              <tr><td>订单应收</td><td>{{ financeSummary.orderCount }}</td><td>HKD {{ money(financeSummary.receivableHKD) }}</td><td>RMB {{ money(financeSummary.receivableRMB) }}</td><td>按订单应收金额汇总</td></tr>
              <tr><td>司机工资</td><td>{{ financeWageRows.length }}</td><td>HKD {{ money(financeSummary.driverPayableHKD) }}</td><td>RMB {{ money(financeSummary.driverPayableRMB) }}</td><td>按司机费用规则、订单代垫和预支/报销汇总</td></tr>
              <tr><td>外派车辆成本</td><td>{{ financeSummary.outsourcedOrderCount }}</td><td>HKD {{ money(financeSummary.supplierPayableHKD) }}</td><td>RMB {{ money(financeSummary.supplierPayableRMB) }}</td><td>供应商规则应付，未匹配规则时用外派车辆代垫兜底</td></tr>
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section v-else-if="activeModule === 'financeDaily'" class="work-page finance-page">
        ...
        <div class="table-card finance-table-card">
          <table class="data-table compact">
            ...
            <tbody>
              <tr><td>客户应收</td><td>全部订单</td><td>HKD {{ money(financeSummary.receivableHKD) }} / RMB {{ money(financeSummary.receivableRMB) }}</td><td>来自订单管理</td></tr>
              <tr><td>司机工资</td><td>全部司机</td><td>{{ moneyPair(financeSummary.driverPayableHKD, financeSummary.driverPayableRMB) }}</td><td>来自司机费用规则、订单代垫和预支/报销</td></tr>
              <tr><td>外派成本</td><td>外派车辆订单</td><td>{{ moneyPair(financeSummary.supplierPayableHKD, financeSummary.supplierPayableRMB) }}</td><td>供应商规则应付，未匹配规则时用外派车辆代垫兜底</td></tr>
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section v-else-if="activeModule === 'freight'" class="work-page">
        ...
        <div class="table-card freight-template-board">
          <div class="table-card-head freight-template-head">
            ...
            <button
              v-if="freightPanelTab !== '收费项目'"
              class="primary-btn small"
              @click="editFreightRate(null); freightRateForm.direction = SHARED_DIRECTION"
            ><IconSvg name="plus" />新增报价</button>
            <button v-else class="primary-btn small" @click="startNewFeeItem()"><IconSvg name="plus" />新增项目</button>
          </div>
...
<div class="freight-batch-toolbar">
          ...
          <div class="freight-directory-summary">
            <strong>进出口通用报价 / {{ freightDirectoryMeta.label }}</strong>
      ...
      <section v-else-if="activeModule === 'rules'" class="work-page">
        <div class="section-head">
          <div>
            <p class="eyebrow">规则管理</p>
            <h2>审核、删除、费用与工资规则</h2>
          </div>
          <button class="primary-btn" @click="editRule(null)"><IconSvg name="plus" />新增规则</button>
        </div>
        <div class="module-grid two-columns">
          <div class="table-card">
            <table>
              <thead><tr><th>类型</th><th>规则名称</th><th>启用</th><th>操作</th></tr></thead>
              <tbody>
                <tr
                  v-for="item in ruleRows"
                  :key="item.id"
                  :class="{ selected: selectedRule?.id === item.id }"
                  @click="selectedRuleId = item.id; editRule(item)"
                >
                  <td>{{ item.ruleType }}</td>
                  <td>{{ item.name }}</td>
                  <td>{{ item.enabled ? '启用' : '停用' }}</td>
                  <td class="row-actions">
                    <button class="icon-btn" @click.stop="editRule(item)"><IconSvg name="edit" />编辑</button>
                    <button class="icon-btn danger" @click.stop="deleteRule(item)"><IconSvg name="trash" />删除</button>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
          <form class="edit-card" @submit.prevent="saveRule">
            <h3>{{ ruleForm.id ? '编辑规则' : '新增规则' }}</h3>
            <label>规则类型<input v-model.trim="ruleForm.ruleType" /></label>
            <label>规则名称<input v-model.trim="ruleForm.name" /></label>
            <label class="inline-check"><input v-model="ruleForm.enabled" type="checkbox" /> 启用</label>
            <label>规则说明<textarea v-model.trim="ruleForm.content" rows="8" /></label>
            <button class="primary-btn"><IconSvg name="save" />保存规则</button>
          </form>
        </div>
      </section>
      ...
      <div v-if="activeOrderDetail" class="modal-backdrop full-detail-backdrop" @click.self="closeOrderDetail">
        <section class="modal-card full-detail-modal">
          <div class="modal-head">
            <div>
              <h2>{{ activeOrderDetail.no }} · 订单明细</h2>
              <p class="modal-subtitle">{{ activeOrderDetail.customer || '-' }} · {{ activeOrderDetail.date || '-' }} · {{ activeOrderDetail.status || '-' }}</p>
            </div>
            <div class="modal-detail-actions">
              <button class="ghost-btn small" type="button" @click="openOrderModal(null, activeOrderDetail)"><IconSvg name="edit" />编辑</button>
              <button type="button" class="icon-btn" @click="closeOrderDetail"><IconSvg name="close" />关闭</button>
            </div>
          </div>
          <div class="modal-body full-detail-body">
            <div class="detail-section-grid">
              <section class="detail-section">
                <h3>基础信息</h3>
                <dl class="detail-dl">
                  <dt>排车单号</dt><dd>{{ activeOrderDetail.dispatchNo || '-' }}</dd>
                  <dt>业务类型</dt><dd>{{ activeOrderDetail.businessType || '-' }}</dd>
                  <dt>口岸</dt><dd>{{ activeOrderDetail.port || '-' }}</dd>
                  <dt>进出口</dt><dd>{{ activeOrderDetail.direction || '-' }}</dd>
                  <dt>吨位</dt><dd>{{ activeOrderDetail.tonnage || '-' }}</dd>
                  <dt>件数/板数</dt><dd>{{ activeOrderDetail.quantity || '-' }}</dd>
                  <dt>重量</dt><dd>{{ activeOrderDetail.weight || '-' }}</dd>
                  <dt>币种</dt><dd>{{ currencyCodeDisplay(activeOrderDetail.currency || '') || '-' }}</dd>
                </dl>
              </section>
              <section class="detail-section">
                <h3>车辆司机</h3>
                <dl class="detail-dl">
                  <dt>车辆来源</dt><dd>{{ activeOrderDetail.vehicleSource || '-' }}</dd>
                  <dt>车牌</dt><dd>{{ activeOrderDetail.plate || '-' }}</dd>
                  <dt>运输模式</dt><dd>{{ normalizeTransportMode(activeOrderDetail.transportMode || '') || '-' }}</dd>
                  <dt>司机</dt><dd>{{ orderDetailDriverText(activeOrderDetail) }}</dd>
                  <dt>外派供应商</dt><dd>{{ activeOrderDetail.supplier || '-' }}</dd>
                  <dt>车次号</dt><dd>{{ activeOrderDetail.tripNo || '-' }}</dd>
                  <dt>六联单号</dt><dd>{{ activeOrderDetail.sixSheetNo || '-' }}</dd>
                  <dt>状态</dt><dd><span class="status-badge" :class="orderStatusClass(activeOrderDetail.status)">{{ activeOrderDetail.status || '-' }}</span></dd>
                </dl>
              </section>
              <section class="detail-section detail-section-wide">
                <h3>装卸地址</h3>
                <dl class="detail-dl detail-dl-wide">
                  <dt>装货地</dt><dd>{{ activeOrderDetail.loading || '-' }}</dd>
                  <dt>卸货地</dt><dd>{{ activeOrderDetail.unloading || '-' }}</dd>
                  <dt>备注</dt><dd>{{ activeOrderDetail.remark || '-' }}</dd>
                </dl>
              </section>
            </div>
            <section class="detail-section">
              ...
              <div class="table-wrap detail-table-wrap">
                <table class="data-table compact">
                  ...
                  <tbody>
                    <tr v-for="(fee, index) in orderDetailFeeRows(activeOrderDetail)" :key="`${fee.name}-${index}`">
                      <td>{{ index + 1 }}</td>
                      <td>{{ fee.name || '-' }}</td>
                    ...
                    <tr v-if="orderDetailFeeRows(activeOrderDetail).length === 0"><td colspan="6">暂无费用明细</td></tr>
                  </tbody>
                </table>
              </div>
            </section>
        ...
        <section class="modal-card full-detail-modal">
              ...
              <details class="data-table-column-menu">
                ...
                <div class="data-table-column-popover finance-wage-detail-column-popover order-list-detail-column-popover">
                  ...
                  >
                    ...
                    ><IconSvg name="lock" /></button>
                    <button class="icon-btn icon-only" type="button" title="上移" :disabled="column.locked" @click.prevent="moveOrderColumn(column, -1)"><IconSvg name="chevronUp" /></button>
                    <button class="icon-btn icon-only" type="button" title="下移" :disabled="column.locked" @click.prevent="moveOrderColumn(column, 1)"><IconSvg name="chevronDown" /></button>
                  </label>
                </div>
              </details>
              <button class="ghost-btn small" type="button" @click="resetOrderColumnOrder"><IconSvg name="list" />恢复列序</button>
          ...
          <div class="modal-body full-detail-body">
            <div class="nested-table-scroll order-list-detail-scroll">
              <table class="data-table compact order-table order-list-detail-table">
                <colgroup>
                ...
                <thead>
                  <tr>
                    <th
                      ...
                      :key="column.key"
                      :class="['resizable-th', { sortable: column.key !== 'actions', sorted: tableSortDirection('orders', column.key), 'sticky-managed-column': isOrderColumnFrozen(column) }]"
                      :style="orderFrozenColumnStyle(column, index)"
                      @click="toggleTableSort('orders', column)"
                    >
        ...
        <form class="modal-card compact-modal customer-edit-modal" @submit.prevent="saveCustomer">
          ...
          <div class="modal-body">
            <div class="customer-modal-tabs">
              ...
              <label class="customer-kind-inline">类型
      ...
      <div v-if="orderModalOpen" class="modal-backdrop">
        <form class="modal-card order-modal-card" @click="dispatchMessageOpen = false; loadFeeTemplateMenuOpen = false; closeRouteTreeDropdown(); orderCustomerPickerOpen = false" @submit.prevent="saveOrder">
          <div class="modal-head">
            <h2>
              {{ editingOrderNo ? '编辑订单' : '新建订单' }}
              <span class="order-title-meta">经营单位：{{ orderForm.customer || "-" }}</span>
              <span class="order-title-meta">订单号：{{ orderModalTitleNo }}</span>
            </h2>
            <button type="button" class="icon-btn" @click="orderModalOpen = false"><IconSvg name="close" />关闭</button>
          </div>
          ...
          <div class="modal-body">
                  ...
                  <tr v-for="(fee, index) in orderFees" :key="index">
                    <td>{{ index + 1 }}</td>
                        ...
                        >
                          ...
                          <input type="file" @change="uploadOrderFeeFile(fee, index, $event)" />
                        </label>
                        <button
                          v-else
                    ...
                    <td><input v-model.number="fee.unitPrice" type="number" min="0" step="0.01" @input="syncFeeAmountFromUnitPrice(fee)" /></td>
	                    <td>
	                    ...
	                    <td><input v-model.trim="fee.remark" placeholder="备注" /></td>
                    <td>
                      <span class="op-group">
            ...
            <div v-else class="file-panel order-attachment-panel">
              <div class="file-toolbar">
                <label v-if="editingOrderNo" class="file-upload-btn"><IconSvg name="upload" />上传订单附件<input type="file" @change="uploadOrderFile" /></label>
                <button v-else class="file-upload-btn is-disabled" type="button" @click="notify('请先保存订单，再上传附件')"><IconSvg name="upload" />上传订单附件</button>
                <button class="ghost-btn small" type="button" @click="openAttachmentRecycleBin"><IconSvg name="archive" />附件回收站</button>
                <span class="hint">{{ editingOrderNo ? '杂费单据、照片、PDF 上传后会自动归档到当前订单。' : '当前订单还没有订单号，请先保存订单后再上传附件。' }}</span>
              </div>
              <template v-if="editingOrderNo">
                <table class="data-table compact">
                  <thead><tr><th>分类</th><th>文件名</th><th>大小</th><th>上传时间</th><th>操作</th></tr></thead>
                  <tbody>
                    <tr v-for="file in orderAttachmentRows" :key="file.id">
                      ...
                      <td class="row-actions">
                        <button class="icon-btn" @click="openStoredFile(file, 'preview')"><IconSvg name="eye" />预览</button>
                        <button class="icon-btn" @click="openStoredFile(file, 'download')"><IconSvg name="download" />下载</button>
                        <button class="icon-btn danger" @click="deleteFile(file, orderAttachmentRows)"><IconSvg name="trash" />删除</button>
                      </td>
                    </tr>
                    <tr v-if="orderAttachmentRows.length === 0"><td colspan="5">暂无订单附件</td></tr>
                  </tbody>
                </table>
              </template>
            </div>
          ...
          <div class="modal-actions">
            <button type="button" class="ghost-btn" @click="orderModalOpen = false">取消</button>
              ...
              <table class="fee-item-table">
                ...
                >
                  ...
                  <tr v-if="sortedFeeItemRows.length === 0">
	                    <td colspan="8" class="empty-action-cell">
                      <span>暂无收费项目</span>
                    </td>
                  </tr>
                </tbody>
            ...
            <div class="form-grid">
              ...
              <label class="vehicle-insurance-upload">大陆保险单据
                <span class="vehicle-insurance-row">
                  <span class="file-upload-btn compact"><IconSvg name="paperclip" />上传<input type="file" @change="uploadVehicleInsuranceFile('大陆保险单据', $event)" /></span>
              ...
              <label class="vehicle-insurance-upload">香港保险单据
                <span class="vehicle-insurance-row">
                  <span class="file-upload-btn compact"><IconSvg name="paperclip" />上传<input type="file" @change="uploadVehicleInsuranceFile('香港保险单据', $event)" /></span>
    ...
    <div v-if="filePreviewOpen && previewFile" class="modal-backdrop file-preview-backdrop">
      <section class="modal-card file-preview-modal">
        <div class="modal-head">
          <h2>
            附件预览
            <span class="order-title-meta">{{ previewFile.filename }}</span>
          </h2>
          <div class="modal-detail-actions">
            <button type="button" class="icon-btn" @click="openStoredFile(previewFile, 'download')"><IconSvg name="download" />下载</button>
            <button type="button" class="icon-btn" @click="closeFilePreview"><IconSvg name="close" />关闭</button>
          </div>
        </div>
        <div class="file-preview-body">
          <img
            ...
            :src="fileEndpoint(previewFile, 'preview')"
